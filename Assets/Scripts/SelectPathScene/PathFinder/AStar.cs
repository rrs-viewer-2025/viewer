    ...
    public List<int> FindPath()
    {
        ...
        while(true)
        {
            ...
            foreach(int neighbourID in roadGraph[n].neighbours)
            {
                ...
                //スタートから隣接道路までのコスト計算
                double G = gScore[n] + Vector3.Distance(roadGraph[n].posi, roadGraph[neighbourID].posi);
                ...
                if(!L1.Contains(neighbourID) || G < gScore.GetValueOrDefault(neighbourID, double.MaxValue))
                {
                    ...
                    // ソートして f が小さいノードを先頭に
                    L1 = L1.OrderBy(id => gScore[id] + Vector3.Distance(roadGraph[id].posi, roadGraph[goalID].posi)).ToList();
                }
            }
        }
    }
